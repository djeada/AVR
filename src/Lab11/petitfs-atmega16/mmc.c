...
#define SELECT() PORT_SEL &= ~(1 << PIN_SEL)  /* CS = L */
#define DESELECT() PORT_SEL |= (1 << PIN_SEL) /* CS = H */
#define MMC_SEL !(PORT_SEL & (1 << PIN_SEL))  /* CS status (true:CS == L) */

#define FORWARD(d)                                                             \
  xmit(d) /* Data forwarding function (Console out in this example) */

void init_spi(void);   /* Initialize SPI port (spi.c) */
void dly_100us(void);  /* Delay 100 microseconds (spi.c) */
BYTE xmit_spi(BYTE d); /* Send a byte to the MMC (spi.c) */
BYTE rcv_spi(
    void); /* Send a 0xFF to the MMC and get the received byte (spi.c) */
...
#if _USE_WRITE
DRESULT
disk_writep(const BYTE *buff, /* Pointer to the bytes to be written
